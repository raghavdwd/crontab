...
export const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      console.error(err);
      setError("Failed to fetch cron jobs. Please make sure the backend is active.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-[#fafafa] py-12 px-6">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-12">
          <div>
            <h1 className="text-2xl font-normal text-black tracking-tight">Overview</h1>
          ...
          <div className="flex items-center gap-3">
            <Button
              variant="outline"
              size="icon"
              onClick={() => { fetchDashboardData(); fetchLogs(); }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <Card className="border border-[#f1f1f4] bg-white shadow-xs rounded-xl overflow-hidden">
            <CardHeader className="p-6 pb-2">
              <CardDescription className="text-xs font-light text-[#71717a] uppercase tracking-wider">Total Scheduled</CardDescription>
            </CardHeader>
            <CardContent className="p-6 pt-0 flex items-baseline justify-between">
              <span className="text-3xl font-light text-black">{jobs.length}</span>
              <span className="text-xs font-light text-[#71717a]">Jobs configured</span>
            </CardContent>
          </Card>

          <Card className="border border-[#f1f1f4] bg-white shadow-xs rounded-xl overflow-hidden">
            <CardHeader className="p-6 pb-2">
              <CardDescription className="text-xs font-light text-[#71717a] uppercase tracking-wider">Active Scheduler</CardDescription>
            </CardHeader>
            <CardContent className="p-6 pt-0 flex items-baseline justify-between">
              <span className="text-3xl font-light text-emerald-600">{activeCount}</span>
          ...
          <Card className="border border-[#f1f1f4] bg-white shadow-xs rounded-xl overflow-hidden">
            <CardHeader className="p-6 pb-2">
              <CardDescription className="text-xs font-light text-[#71717a] uppercase tracking-wider">Recent Success Rate</CardDescription>
            </CardHeader>
            <CardContent className="p-6 pt-0 flex items-baseline justify-between">
              <span className="text-3xl font-light text-black">{successRate}%</span>
              <span className="text-xs font-light text-[#71717a]">Based on latest logs</span>
            </CardContent>
          </Card>
        </div>
            ...
            {loading ? (
              <div className="flex h-60 items-center justify-center bg-white border border-[#f1f1f4] rounded-xl shadow-xs">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-xs font-light text-[#71717a]">Fetching schedules...</p>
                </div>
              </div>
            ) : jobs.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-80 bg-white border border-[#f1f1f4] rounded-xl text-center p-8 shadow-xs">
                ...
                <h3 className="text-base font-normal text-black">No scheduled jobs</h3>
                ...
                <Table>
                  <TableHeader className="bg-neutral-50/50">
                    <TableRow className="border-b border-[#f1f1f4] hover:bg-transparent">
                      <TableHead className="font-normal text-neutral-500 text-xs w-[200px] h-11 px-6">Name</TableHead>
                      <TableHead className="font-normal text-neutral-500 text-xs w-[140px] h-11 px-6">Schedule</TableHead>
                      <TableHead className="font-normal text-neutral-500 text-xs h-11 px-6">Target Hook Endpoint</TableHead>
                      <TableHead className="font-normal text-neutral-500 text-xs w-[120px] h-11 px-6">Status</TableHead>
                      <TableHead className="font-normal text-neutral-500 text-xs w-[120px] h-11 px-6 text-right">Actions</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {jobs.map((job) => {
                      ...
                      return (
                        <TableRow key={job._id} className="border-b border-[#f1f1f4] hover:bg-neutral-50/30 transition-colors">
                          ...
                          <TableCell className="px-6 py-4 font-mono text-xs text-[#71717a] max-w-[300px] truncate">
                            <span className="flex items-center gap-1.5">
                              <span className="text-[10px] uppercase font-bold text-neutral-400">GET</span>
          ...
          <TabsContent value="logs" className="focus-visible:outline-none">
            {logsLoading ? (
              <div className="flex h-60 items-center justify-center bg-white border border-[#f1f1f4] rounded-xl shadow-xs">
                <div className="flex flex-col items-center gap-2">
                  <div className="h-5 w-5 animate-spin rounded-full border-2 border-black border-t-transparent" />
                  <p className="text-xs font-light text-[#71717a]">Streaming runtime logs...</p>
                </div>
              </div>
            ) : logs.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-60 bg-white border border-[#f1f1f4] rounded-xl text-center p-8 shadow-xs">
                ...
                <h3 className="text-base font-normal text-black">No run history</h3>
                <p className="text-xs font-light text-[#71717a] mt-1 max-w-sm">
                  Run history logs will appear here once your active cron schedules execute pings.
                </p>
              </div>
            ) : (
